...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                this.beforeComunication(clientSocket, inputLine);
                
                if (this.shouldFinish(inputLine)) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
           this.onConnectionEnd(clientSocket);
        } catch (IOException e) {
    ...
    protected void beforeComunication(Socket clientSocket, String inputLine) {
    	System.out.println("Received message: " + inputLine + " from "
                + clientSocket.getInetAddress().getHostAddress() + ":" + clientSocket.getPort());
    }
    protected void onConnectionEnd(Socket clientSocket) {
    	 System.out.println("Connection closed with " + clientSocket.getInetAddress().getHostAddress() + ":"
                 + clientSocket.getPort());
    }
    protected boolean shouldFinish(String input) {
    	return (input.equalsIgnoreCase(this.getTerminationWord()));
    }
    protected String getTerminationWord() {
    	return "Pinga";
    }
}
